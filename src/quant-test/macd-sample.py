...
def macd(signals):
    ...
    signals['ma1'] = signals['Close'].rolling(
        window=ma1, min_periods=1, center=False).mean()
    signals['ma2'] = signals['Close'].rolling(
        window=ma2, min_periods=1, center=False).mean()
...
def signal_generation(df, method):
    ...
    # positions becomes and stays one once the short moving average is above
    # long moving average
...
def plot(new, ticker):
    ...
    ax.plot(new.loc[new['signals'] == 1].index, new['Close']
            [new['signals'] == 1], label='LONG', lw=0, marker='^', c='g')
    ax.plot(new.loc[new['signals'] == -
                    1].index, new['Close'][new['signals'] == -
                                           1], label='SHORT', lw=0, marker='v', c='r')
...
def main():
    ...
    # so it is tw   o week moving average versus one month moving average
